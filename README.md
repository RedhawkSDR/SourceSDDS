...
| ------------- | -----|
| buffer_size | The maximum number of elements (SDDS Packets) which can be held within the internal buffer. If there is up stream back pressure this buffer will start to fill first and provide pressure on the socket buffer if full.  Current fullness is displayed within status struct |
