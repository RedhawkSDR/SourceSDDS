...
// Assume we're little endian (x86, Tru64)
...
//  uint16_t d[512];
  ...
  // The set/get_rate() routines interact with the header cx
  ...
  void   set_rate(double val) {
	  ...
	  if ( val < 125e6 ) {
  	  ...
  	  freq = swap8( uint64_t(val * 73786976294.838211) ); // 2^63/125 MHz
  }

  // USE get_rate(), set_rate() instead
  // These get/set the SDDS freq field directly
  // Still provided in case anybody uses them
